function isInside = isin(obj, mesh)

isInside = zeros(length(obj.nodes), mesh.nrMeshes);
for i=1:length(obj.nodes)
    for j=1:mesh.nrMeshes
	% Scale both vertices and nodes to nm space
	vertices = bsxfun(@times, mesh.vertices{j}, mesh.scale);
	nodes = bsxfun(@times, obj.nodes{i}(:,1:3), obj.scale);
        isInside(i,j) = any(inpolyhedron(mesh.faces{j}, vertices, nodes));
    end
end
