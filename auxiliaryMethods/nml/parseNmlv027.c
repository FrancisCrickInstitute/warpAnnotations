...
/* Copyright 2013, 2014, 2015, 2016 Max Planck Institute for Brain Research, Frankfurt                   */
/* Version 0.27                     Martin Zauser                                                        */
/* ***************************************************************************************************** */
...
#define VERSION       "0.27"
#define VERSION_DATE  "28.04.2016"
...
unsigned char szValueBuffer[VALUE_BUFFER_SIZE];
...
void mexFunction (int nlhs , mxArray *plhs[], int nrhs , const mxArray *prhs[]) {
    ...
    unsigned char szPathCommentFiles[MAX_LENGTH_FILENAME + MAX_LENGTH_PATH + 1 + 6]; /* plus \*.nml => 6 characters */
    unsigned char szPathAndFilenameCommentFiles[MAX_LENGTH_FILENAME + MAX_LENGTH_PATH + 1 + MAX_LENGTH_FILENAME + 1];
    unsigned char szPathAndFilenameOutput[MAX_LENGTH_FILENAME + MAX_LENGTH_PATH + 1];
    ...
    unsigned char szTaskId[LENGTH_MONGODB_ID + 1]; /* the task id is a typical mongoDB is with 12 bytes = 24 hex characters (plus 1 for the trailing zero) */
    ...
    printf("This is Braintracing NML parser version %s, Copyright 2016 MPI for Brain Research, Frankfurt.\n", VERSION);
            ...
            if ((iSelectMode != 0) && (nlhs == 0)) {
                ...
                if ((iListComments != 0) || (iSelectMode != 0)) {
                    if (!(windowsFindData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
                    {
                        ...
                        if (iListComments != 0) {
                            iCountCharacters = 0;
                            for (i = ((int)strlen(szPathAndFilenameCommentFiles) - 1); i >= 0; --i) {
                    ...
                    do {
                        ...
                        if ((strcmp(szNmlElement, "edge") == 0) || (strcmp(szNmlElement, "edge/") == 0)) {
                            ...
                            if (iSelectMode == 0) {
                                ...
                                if (iIllegalEdge == 0) {
                                    for (m = iNumberOfNodes - iNumberOfNodesThing[nmlThingCounter]; m < iNumberOfNodes; ++m) {
                ...
                if (nlhs == 0) {
                    ...
                    for (k = 0; k < nmlThingCounter; k++) {
                        ...
                        for (i = 0; i < iNumberOfNodesThing[k]; i++) {
                            ...
                            do {
                                iNodeExists = 0;
                                for (j = i + 1; j < iNumberOfNodesThing[k]; j++) {
                            ...
                            for (j = 0; j < iNumberOfEdgesThing[k]; j++) {
                                for (m = 0; m < NUM_OF_EDGE_ATTRIBUTES; m++) {
    ...
    /* TEST       } else {
            ...
            for (k = 0; k < nmlThingCounter; k++) {
                ...
                if (iNumberOfNodesThing[k] > 0) {
                    ...
                    mwPointer = 0;
                    for (j = 0; j < (NUM_OF_NODE_ATTRIBUTES_ALL - 1); j++) { /* ignore last column (comment) */
        ...
        /* the following solution checks memory size AFTER using it which could raise a memory violation exception in case memory isn't big enough */
                ...
                if (iListComments != 0) {
                    for (i = 0; i < nmlCommentCounter; i++) {
                        ...
                        printf("%s%s%sNode %d (%.0f,%.0f,%.0f): %s\n",
                                ...
                                iCommentNodeID[i], dNode[iNodeIdConversionAllThings[iCommentNodeID[i]]][2],
                                dNode[iNodeIdConversionAllThings[iCommentNodeID[i]]][3], dNode[iNodeIdConversionAllThings[iCommentNodeID[i]]][4],
                                pCommentContent[i]);
                    }
                }
                /* ....................................................................... */
            ...
            if ((iNumberOfBranchpoints > 0) && (iSelectMode == 0)) {
                ...
                for (i = 0; i < iNumberOfBranchpoints; i++) {
                    ...
                    for (j = 0; j < strlen(szMessageBuffer); j++) {
